...
class Blogs extends Component {
    ...
    submitSearchForm = (event) => {
        ...
        axios.get(`https://travellog-7th-backend.herokuapp.com/blogs${this.state.search}`)
            .then(res => {
                const blogslist = res.data;
                this.setState({ blogslist, currentPage: 1 });
                alert('Click choose page')
            })
    }
    ...
    render() {
        ...
        const renderPageNumbers = pageNumbers.map(number => {
            return (
                <li key={number} id={number} onClick={this.handleClick} className='page-link' style={{ cursor: 'pointer' }}>
                    {number}
                </li>
            );
        });
            ...
            <div >
                ...
                <div className="search-box">
                    <div className="input-group search">
                        ...
                        <select onChange={(event) => this.isInputChange(event)} className="custom-select" type="text" id="search" name="search">
